...
/**
 * Driver générique pour tout fournisseur compatible API OpenAI (Groq, DeepSeek,
 * Mistral, OpenRouter, Ollama, xAI, Fireworks, Together…). La base_url est
 * paramétrable — un seul adaptateur couvre tous ces fournisseurs.
 */
class OpenAiCompatibleProvider implements AiProvider
{
    ...
    public function run(string $jobType, DocumentVersion $version, array $params = []): array
    {
        ...
        if ($response->failed()) {
            throw new \RuntimeException($this->providerName.' : '.$response->status().' '.mb_substr($response->body(), 0, 200));
        }
        ...
        if (! is_array($parsed)) {
            throw new \RuntimeException($this->providerName.' : réponse JSON invalide.');
        }
